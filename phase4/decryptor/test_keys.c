...
int main(int argc, char **argv){
    ...
    char *encrypted_fname = argv[3];
    char *decrypted_fname = argv[4];

    int fd_encrypted = open(encrypted_fname, O_RDONLY);
    ...
    while(i <= OBF_KEY_SIZE-32){
        ...
        int fd_decrypted = open(decrypted_fname, O_WRONLY | O_TRUNC | O_CREAT, 0777);
        ...
        snprintf(cmd, 200, "file %s", decrypted_fname);
